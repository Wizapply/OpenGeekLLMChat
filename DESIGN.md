...
97. **Qwen3.6 35B-A3B[MoE] の `<think>` タグ未閉鎖による独白リーク**
    ...
    **根本原因: システムプロンプトに書いた `</` + `think>` をモデルが引用した**
    実機のスクリーンショットで判明。思考ブロックが `output only the final response after \`` で途切れ、
    本文が `` `. `` で始まっていた。これは **モデルが `meta` プロンプトの指示を思考内で言い換えた際、
    閉じタグの文字列をそのまま書いた**ため。パーサ (`/^<think>([\s\S]*?)(<\/think>)?([\s\S]*)$/`) は
    非貪欲マッチなので **引用された最初の1つを本物の閉じタグとみなして分割**し、
    残りの思考（`2. Deconstruct Request:` 以降）が全部本文に落ちた。
    先頭の `` `. `` は、モデルが書いた `` `</think>`. `` の閉じバッククォートとピリオドの残骸。

    → **プロンプトからタグの文字列そのものを排除**する。「思考パートの中で済ませる」と
    日本語で説明すれば意図は伝わり、モデルが引用してもタグ文字列は生まれない。
    プロンプトに制御トークンの綴りを書くと、モデルがそれを復唱してパーサと衝突する。

    **同じ罠の別形: 指示に使った語が定型句として出力される**
    「結論から書き始めてください」と書いたところ、モデルが毎回
    「結論から言うと、」で書き出すようになった。指示の語彙はそのまま出力の語彙になる。
    **書き方を指示するときは、出力に現れてほしくない語を使わない**。
    「1文目が答えそのものになるように」「毎回同じ決まり文句で書き出さない」のように、
    振る舞いだけを述べて、なぞれる言い回しを与えないこと。

    **追加対処: 見出し形式の「計画メモ」リーク (`splitLeakedReasoning`)**
    上の独白パターンは【文単位】なので、思考が次のような見出し+箇条書きで漏れると1行も一致せず素通りする:
    ```
    2. Deconstruct Request:
    * Part 1: RDMA specifications/overview
    3. Knowledge Retrieval & Structuring (Internal):
    ```
    文面ではなく**構造**で判定する検出を足した:
    - **前提**: 直近のユーザー入力に日本語が含まれる時だけ動く (`lastUserTextRef`)。英語で聞かれたら英語で答えるのが正しいため
    - **切り出し**: 思考は先頭だけに出るとは限らない。実機では `[英語の計画メモ] → [日本語の回答] → [英語の自己検証メモ]` と本文を挟み込む形で現れたため、**全域を走査**して「日本語を1文字も含まない連続した塊」ごとに判定する。コードフェンスの中身は英語で正常なので常に保持する
    - **位置による強弱**: 先頭の塊は計画メモが出やすいので条件を緩め、本文の途中・末尾に現れる英語塊は正当な技術用語や引用のことがあるため強い目印を必須にする
    - **確定条件**: 候補ブロックに強い目印 (`(Internal)`, `Deconstruct`, `Knowledge Retrieval`, `I will`, `The user`, `Let me` 等) があるか、3行以上あって見出し/箇条書きが2行以上ある場合のみ退避する。英語の見出しや用語で始まる**正当な回答を消さない**ため、目印を必須にしている
    - **救済処理との整合**: content が空になった時の昇格処理でも「日本語を含まない行は本文ではない」を条件に加える。これが無いと、退避した計画メモがそのまま本文に昇格して元に戻ってしまう

    なお根本対処は llama-server 側で思考を構造的に分離させること (`--jinja --reasoning-format deepseek`)。
    サーバーが `reasoning_content` に切り出してくれれば、クライアントは推測せずに済む (`delta.reasoning_content` の処理は実装済み)。
    上記の検出は、それでも漏れた場合の最後の砦。

97-b. **`enable_thinking: false` は効かないモデルがある → ツール判断が静かに失敗する**
    ツール判断は `chat_template_kwargs: { enable_thinking: false }` で thinking を切って高速化している。
    しかしこのパラメータは **モデル世代によって無視される**。Qwen3.6 では効いたが、
    Qwen3.8-27B では思考が止まらず、次の連鎖で判断が壊れた:

    1. 判断の思考だけで `max_tokens` (= `smallPredict`) を使い切る
    2. `tool_calls` を出す前に `finish_reason: 'length'` で打ち切られる
    3. 受け側は `choice?.message?.tool_calls || null` を見るだけだったので、
       **「ツール不要」と解釈して素通り** → 検索すべき質問が検索されない

    症状は「なぜかツール判断が遅く、精度が落ちた」としか見えず、原因に辿り着けない。
    llama.cpp 側にも既知の問題がある (ggml-org/llama.cpp#20182)。

    **対処**: 打ち切りを検出して一度だけ予算を増やして引き直す。
    - `finish_reason === 'length'` かつ `tool_calls` が空 → `agentContext.judgeRetryPredict`
      (既定 `max(largePredict, smallPredict*4)`) で再実行
    - 既に上限で回している時 (`needsLargeGen`) は引き直さない (同じ結果になるため)
    - 増やしても打ち切られたら、`--reasoning-budget 0` を促す警告をコンソールに出す。
      **静かに失敗させないことが肝**で、次にモデルを入れ替えた時も同じ罠に嵌らない

    根治したい場合は llama-server に `--reasoning-budget 0` を渡す (全リクエストで thinking が止まる)。

97-d. **オブジェクトスプレッドの順序で `max_tokens` が上書きされ、判断が遅かった**
    ツール判断のリクエストが次の順で組まれていた:
    ```javascript
    max_tokens: judgeBudget,        // smallPredict (1024) のつもり
    chat_template_kwargs: {...},
    ...llamaCommonOptions,          // ← ここに max_tokens: largePredict が入っている
    ```
    後勝ちなので **実際に送られていたのは `largePredict` (8192→16384)**。
    `smallPredict` は一度も効いていなかった。thinking が止まらないモデルでは
    判断が上限いっぱいまで考え続け、体感で「ツール判断が長い」となる。

    ログが `console.log('[ツール判断] max_tokens=' + judgeNumPredict)` と
    **送信値ではなく意図した値**を表示していたため、突き合わせても気づけなかった。

    **対処**:
    - `...llamaCommonOptions` を先に置き、`max_tokens` を後ろに書く
      (最終応答側には「llamaCommonOptions より後ろに置くこと」というコメントが既にあった。
       同じ罠を判断側だけ踏んでいた)
    - ログは実際に送る値を出す
    - 判断は分類なので `judgeTemperature` (既定0.1) で決め打ちに寄せる。
      温度が高いと前置きを書いてから答えるぶん遅い
    - `judgeHistoryChars` (既定800) で過去メッセージを切る。
      上限を上げた今、直前の回答が1万トークン超のこともあり、
      判断のたびにそれを再処理していた (最新の質問は切らない)

97-c. **最終応答に tools を渡さないのに、ツール案内はシステムプロンプトに残っている**
    最終応答のリクエスト (`finalRes`) は `tools` を渡さない。ツール判断は済んでいる前提だから。
    しかしシステムプロンプト (`agentSystem`) には `systemPrompts.webSearch` 等の
    ツール案内が載ったままなので、**ツールが1つも実行されなかったターン**では
    「検索すべきだが呼び出せない」という矛盾した状態になる。
    モデルはこれを **検索クエリを本文に書く** ことで解決してしまう。

    実例: 「MPIライブラリで書けますか？」への回答本文が
    `mpi4py MPI-3 one-sided RMA MPI_Win_create support` の1行だけになった
    (検索は実行されておらず、思考だけで973トークン消費していた)。

    既存のテキスト→tool_call 変換フォールバック (パターン1〜3) は
    **関数名を含む** テキストしか拾えない。この形は引数だけなので1つも一致しない。

    **対処は2段構え**:
    - **予防**: 最終応答のシステムプロンプトに
      「この応答ではツールを呼び出せません。検索クエリではなく回答本文を書いてください」を追記して矛盾を解消する
    - **救済**: それでもクエリだけが出たら `looksLikeSearchQuery()` で検出し、
      ツールが使えない旨を伝えて再生成する。既存の「空応答の救済処理」に相乗りする形

    **ツール実行の有無で場合分けしないこと**（一度ここで間違えた）。
    最初の実装は「ツールを1件も実行していないターン」に限定していたが、実際には
    **検索が空振りした後こそ再発する**。5回検索して3回が0件だったターンで、
    モデルはもう一度引こうとして `mpi4py readthedocs one-sided communication RMA tutorial`
    と本文に書いた。最終応答に tools が無いのは常に真なので、条件を付ける理由がない。
    ツール結果がある場合は「追加の検索はできない。得られた結果と知識で、
    確認できた範囲と未確認の範囲を切り分けて書く」と促す文面に切り替える。

    `looksLikeSearchQuery()` は正当な短い英語回答を潰さないよう条件を厳しくしている:
    日本語で聞かれている / 1行 / 日本語を含まない / 文や見出しとして終わっていない /
    コード・箇条書き・表でない / 2〜16語。
...
120. **数式の照合は「体裁の揺れ」を吸収しないと、正しい引用まで不一致になる**
     回答の `$$0 \leq s_0(X) \leq H \text{ のとき, } \quad p_0(X) = k_1\{s_0(X)\}^{n_1}$$` が
     資料の `p_0(X) = k_1\{s_0(X)\}^{n1}` と一致しなかった実例。原因は2つあり、
     どちらも RAG の取得失敗ではない (資料側の式が照合パネルに出ている時点で、
     その本文は手元に届いている):
     (a) **条件と説明と式を1本の数式に詰め込んだ。** 資料は式単位で載っているので、
         `\text{ のとき, }` を含む長い文字列は資料のどこにも存在しない。
     (b) **添字の体裁の違い。** 回答は `n_1`、資料の OCR は `n1`。`normalizeMath` は
         `\left \right \quad {}` と空白しか落としていなかったため、`_` 1文字で不一致になる。
     対策は3つ: `normalizeMath` で `\text{…}` を中身ごと落とす、`_` と `^` を落とす
     (位置の情報は文字の並び順に残るので `k_1 s_0` と `k_0 s_1` は今も区別される)、
     そして `\text{…}` と `\quad` を切れ目に分解した断片が全て資料にあれば一致とみなす保険。
     ただし資料側に条件が載っていなければ保険も効かないので、本命は
     `systemPrompts.math` の「1つの `$$ … $$` に入れるのは式1本だけ、条件は地の文へ」。
     照合を緩める方向の変更なので、資料に無い式 (`^{\infty}` への化け) が
     今も `⚠ 資料と不一致` になることを確認してから入れること。

121. **生成中にチャットを切り替えると、ストリームが切り替え後のチャットに流れ込む**
     ストリーミングの更新は `setMessages(prev => ...)` で書かれているので、途中で
     `loadChat` が走ると **prev が「読み込んだ別のチャットの配列」になり**、
     以降のトークンがそちらに追記される。しかも自動保存は `isLoading` 中は動かないので、
     生成が終わった瞬間に「今開いているチャット」としてまとめて保存され、
     別のチャットに回答が紛れ込んだ状態が永続化される
     (`saveChat(chatId, messages, ...)` の `chatId` も切り替え後のIDになる)。
     対策は「移動の前に確認し、続行するなら先に `stopGeneration()` してから移動する」。
     中断してから移動すればストリームは止まっているので、書き込み先がずれない。
     入口は4つあり、全部塞がないと意味が無い:
     チャット履歴のクリック (同じチャットでも `loadChat` で読み込み直すのでガードする) /
     「+ 新規」・チャット削除 / ブラウザの戻る・進む / 別ページへのリンクとリロード。
     戻る・進むは `popstate` が飛んだ時点でURLが変わっていて事前に止められないので、
     中止を選んだ時は `history.pushState` でURLを押し戻し、元のチャットに留める。

122. **`beforeunload` の判定は state ではなく ref で持つ**
     `useEffect([isLoading])` でハンドラを付け替える書き方だと、アプリ側の確認ダイアログで
     「移動する」を選んだ直後はまだ `setIsLoading(false)` が反映されておらず、
     ブラウザ標準のダイアログが二重に出る。`isLoadingRef` を持ち、`stopGeneration` が
     同期でこれを false にしてから移動させれば二重にならない。
     なお `beforeunload` の文言は Chrome/Firefox とも独自メッセージを無視するので、
     日本語で理由を説明したい導線は `preventDefault` できるアプリ側 (onClick) で受ける。
...
## 🧬 RAGから教師データを作る設計 (rag_tune.js)

永続RAGに登録済みの資料から、ファインチューニング用の教師データ (Q&A) を LLM に作らせる仕組み。
`/tuning.html` の「🧬 RAGから生成」タブと `POST /tuning/rag/jobs` で動く。

### なぜ RAG を素材にするのか

- **チャンクが既に意味のまとまりになっている**: OCR/HTML取り込みが Markdown 化して 500文字で割った結果がそのまま使える。改めて前処理を書く必要がない
- **出典が最初から付いている**: OCRジョブが埋めた `pages` があるので、「どの資料の何ページ由来のQ&Aか」を教師データに持たせられる。不良サンプルを追跡できる
- **選別が済んでいる**: RAGに入れた = そのモデルに答えてほしい知識、という判断が既に人間側で終わっている
- **同じ素材から2種類の学習データが出せる**: 資料を渡さない形 (closed) と、検索結果の体裁で渡す形 (open)

### 全体の流れ

```
ml/rag/<docId>.json                       ← 永続RAG (chunks + pages + embeddings)
   │  chunks を passageChunks 個ずつ連結 (overlap ぶんを削る)
   ▼
パッセージ (1500文字前後の自己完結した塊)
   │
   ├─① 生成プロンプト  → [{"q","a"}, ...]          LLM 1回
   │      ▼
   │   機械チェック (指示語 / 数値の裏取り / 3-gram被覆率 / 重複)   ← LLMを呼ばない安いフィルタ
   │      ▼
   ├─③ 検証プロンプト  → {"ok":true|false,"reason"}  LLM 1回 × 生き残った件数
   │
   └─② 拒否例プロンプト → ["資料に無いことを聞く質問", ...]  LLM 1回 (refusalEveryNth ごと)
          ▼  回答は refusalAnswers の定型文に差し替え
   ▼
ml/ragtune/out/<jobId>.jsonl              ← 採用/不採用 + 判定理由つきで全件保存 (レビュー用)
   │  UIで採用/不採用を手直し → 「学習データへ取り込む」
   ▼
tuning/samples.jsonl                       ← 既存の学習サンプルDB (そのまま LoRA SFT へ)
```

### パッセージ化 — なぜチャンクをそのまま使わないか

検索用のチャンクサイズ (既定500文字) は **embedding の ctx (BERT系は512トークン)** に合わせた値であって、
設問を作るのに十分な文脈とは別物。500文字だけ渡すと

- 「この値は」「上記の条件では」と、前のチャンクにある主語や定義が欠けたまま設問が作られる
- 表の途中・手順の途中で切れていて、答えが完結しない

ので、`passageChunks` 個 (既定3) を連結して 1500文字前後にしてから投げる。
連結時は `overlap` ぶんを削る (`ragSearch` の近傍連結と同じ理屈)。削らないと同じ文が二重に入り、
**繰り返しを学習してしまう**。

`minPassageChars` 未満のパッセージ (目次の残り・奥付・空ページ) は捨てる。
ここから良い設問はまず出ず、LLM呼び出しと検証だけが無駄にかかるため。

### プロンプト設計（本機能の中心）

プロンプトは3本 + 学習サンプル用の system が2本。すべて `config.json` の `tuning.ragDataset` にあり、
UI のテキストエリアからジョブ単位で上書きできる。

#### ① 生成プロンプト (`generatePrompt`)

展開される変数: `{passage}` `{source}` `{n}` `{styles}` `{minChars}` `{maxChars}`

ルールはどれも「実際にやらかす失敗」への対処になっている:

| ルール | なぜ必要か |
|---|---|
| 資料に書かれていないことは書かない（推測禁止） | LLMは一般常識で穴を埋める。埋めた内容を学習すると、モデルは**社内資料の顔をした一般論**を喋るようになる |
| 質問に「この資料」「上記」等の指示語を使わない | 学習時の入力は質問文だけ（closed）。指示語入りの質問は推論時に指す先が無く、そのまま学習すると「資料を見た前提の受け答え」が癖になる |
| 固有名詞を質問文に必ず入れる | 「重量は？」では何の重量か決まらない。複数の製品を学習させたとき、答えが混ざる |
| 回答にも「資料によると」を書かせない | メタ表現を学習すると、**RAGを使っていない場面でも**「資料によると」と言い出す |
| 数値・単位・型番は表記どおりに写す（丸めない・換算しない） | 1.2kg → 約1.2キロ のような書き換えは、原典照合ができなくなるうえ誤差が固定される |
| 回答の文字数を `{minChars}`〜`{maxChars}` に収める | 短すぎ＝単語だけの教師データ、長すぎ＝パッセージの丸写しになりやすい |
| 質問の種類を散らす (`{styles}`) | 事実確認ばかりだと「〜は何ですか」形式にしか反応しないモデルになる。定義・手順・条件・原因・トラブル対応を混ぜる |
| 同じ事実は聞き方を変えて複数回 | **同じ知識を多様な表現で繰り返す**のが知識注入の基本。言い換えのバリエーションが少ないと、少し違う聞き方で答えられない |
| 使える情報が無ければ `[]` を返してよい | 「何か作らなければ」と思わせると、目次からでも無理やり設問をひねり出す |
| JSON配列のみ・前置きなし | パーサを単純に保つため。ただし実装側は壊れた出力も拾う（後述） |

#### ② 拒否サンプルのプロンプト (`refusalPrompt`)

**資料の話題には近いが、資料には答えが書かれていない質問**を作らせる。回答はLLMに作らせず、
`refusalAnswers` の定型文からランダムに割り当てる（LLMに断り文句を作らせると、断りながら
一般知識で答えてしまう個体がいるため）。

これが無いと、学習後のモデルは**範囲外の質問にも堂々と作文する**ようになる。
全体の1割程度が目安で、`refusalEveryNth` (既定3パッセージに1回) で間引く。
入れ過ぎると何でも「記載がありません」と答えるモデルになる。

#### ③ 検証プロンプト (`verifyPrompt`)

展開される変数: `{passage}` `{question}` `{answer}`

生成したQ&Aを**別の呼び出しで**判定させる。同じ生成の続きで自己採点させても、
直前に自分が書いた文章を肯定するだけで意味がない。判定基準は4つ:

1. 回答の内容がすべて資料に書かれているか（足していないか）
2. 数値・単位・固有名詞が資料と一致しているか
3. 質問が資料を見なくても意味が通るか
4. 回答が質問に正面から答えているか

出力は `{"ok": true, "reason": ""}` の JSON。**判定不能（JSONを返せない小型モデル）は通す**。
ここで落とすと 3B 以下では全滅し、機械チェックだけの方がまだ精度が出るため。

#### 学習サンプルの形 — closed と open

同じQ&Aから2種類の学習データが作れる。**どちらが要るかは運用方針で決まる**。

**closed（知識注入型）** — 資料を渡さず、質問だけで答えさせる:

```json
{"system": "あなたは社内ドキュメントに詳しいアシスタントです…",
 "instruction": "OG-100 の本体重量は何kgですか？",
 "response": "本体重量は1.2kgです。"}
```

RAGなしで答えさせたいとき用。ただし**知識注入は件数が要る**（同じ事実を言い換えで何度も）。
数十件では覚えない。

**open（RAG運用型）** — 検索結果と同じ体裁で資料を添えて答えさせる:

```json
{"system": "あなたは社内ドキュメント検索の回答役です…",
 "instruction": "── 資料 S1 ──\n出典キー: S1   (OG-100.md p.3)\n<パッセージ原文>\n── ここまでが S1 の内容 ──\n\n…\n【質問】\nOG-100 の本体重量は何kgですか？",
 "response": "本体重量は1.2kgです。【S1】"}
```

`contextTemplate` の体裁は、チャットが `search_persistent_documents` の結果をLLMへ渡すときの形
（`── 資料 S1 ──` / `出典キー: S1` / 末尾に【S1】）に**意図的に合わせてある**。
学習時と推論時で入力の見た目が違うと、せっかく覚えた「資料を読んで答える型」が本番で発火しない。
チャット側のフォーマットを変えたら、このテンプレートも合わせて変えること。

open は少量でも効く（型を教えるだけなので）。**出典の付け方**と**資料に無ければ断る**挙動が同時に入るのが利点。

`both` は同じQ&Aから両方を作る。件数は倍になる。

### 品質フィルタ — 機械チェックを先に、LLM検証を後に

検証はサンプル1件につきLLM1回かかる。1000件作れば1000回。
そこで**LLMを呼ばずに落とせるものは先に落とす**:

| チェック | 落とすもの | 実装 |
|---|---|---|
| 指示語 | 「この資料の〜」を含む質問 | 正規表現 `DEICTIC_RE` |
| メタ表現 | 「資料によると」を含む回答 | 正規表現 `META_RE`（拒否サンプルは対象外） |
| 数値の裏取り | 資料に存在しない数値を含む回答 | 全角→半角・桁区切り除去してから突き合わせ |
| 被覆率 | 回答の文字3-gramが資料にほとんど無い（＝一般論） | `coverage()` が `minCoverage` 未満で不採用 |
| 重複 | 既出の質問と同じ／ほぼ同じ | 正規化一致 + 文字3-gram Jaccard ≧ `dedupeThreshold` |
| 長さ | 短すぎる・長すぎる回答 | `minAnswerChars` / `maxAnswerChars` |

日本語は分かち書きが要るので、**単語ではなく文字3-gram**で見ている（形態素解析の依存を増やさないため）。

不採用のサンプルも**理由付きで残す**（`status: 'rejected'`, `reason`）。捨ててしまうと
「プロンプトが悪いのか、フィルタが厳しすぎるのか」が分からなくなる。UIで理由を見ながら
しきい値やプロンプトを調整し、手動で採用に戻すこともできる。

### LLM出力のパース

`parseQaList()` は次の順で拾う。**きれいなJSONしか受け付けない実装にすると歩留まりが落ちる**ため:

1. 素直に `JSON.parse`（配列 / `{items:[...]}` / 単体オブジェクト）
2. 最初の `[` 〜 最後の `]` を切り出して parse（前置き・後書きを捨てる）
3. `{...}` を1件ずつ拾う（`max_tokens` で途中で切れた配列・JSONL形式に効く）
4. それも駄目なら素の文字列配列を拾う（拒否例プロンプトの出力）

キー名のゆれ (`q`/`question`/`instruction`、`a`/`answer`/`response`) も吸収する。
`<think>…</think>` とコードフェンスは先に剥がす。

### ストレージとジョブ

```
ml/ragtune/
├── jobs.json              # ジョブ状態 (再起動で復元。実行中だったものは待機中に戻す)
└── out/<jobId>.jsonl      # 生成結果 (1行1サンプル)
```

生成結果の1行:

```json
{"id":"rs_…","kind":"qa","question":"…","answer":"…","context":"<パッセージ原文>",
 "source":{"docId":"…","filename":"manual/OG-100.md","page":3,"chunkRange":[0,2],
           "passageKey":"<docId>:0-2","label":"OG-100.md p.3"},
 "status":"accepted","reason":"","checks":{"coverage":0.82,"verified":true},"createdAt":…}
```

- **`context` を各行に持たせる**: 取り込み時に closed / open のどちらにも展開できるようにするため。
  パッセージ原文を後から引き直す必要がない（ドキュメントが再登録されて内容が変わっても、
  生成時に使った本文で再現できる）
- **`passageKey` で再開できる**: ジョブ再実行時、出力JSONLに既にあるパッセージは飛ばす。
  数百パッセージを回す処理で「中断したら最初から」は実用にならない
- **取り込みはコピー**: `tuning/samples.jsonl` に追記するだけなので、ジョブを削除しても
  取り込み済みの学習データは消えない

### LLMの確保

OCR の Vision LLM と同じ考え方:

- `tuning.ragDataset.poolModel` に `chatModels` の名前を入れると **LLMプール管理**。
  生成中だけロードされ、終われば `orchestration.idleUnloadMs` でアンロードされる。
  単一GPUでメインチャットと同居できない場合は `planMode()` の判定で swap になる
- 空なら **メインチャットの llama-server をそのまま叩く**（既に載っているモデルで生成する）。
  設定ゼロで動かせる代わりに、生成中はチャットが待たされる

生成は `stream: false`。JSONを丸ごと受け取って parse するだけなので、途中経過に意味がない。

### 件数が伸びないときの構造（1ページあたり何問になるか）

「300ページの資料から64件しか採れない」は、次の3つが重なって起きる:

1. **パッセージがページより粗い** — チャンク単位 (500文字 × 3) だと、1ページ1チャンクの資料では
   3ページ分が1パッセージになる。300ページ → 100パッセージ。設問数の上限がページ数より減る
2. **モデルが指定数を返さない** — 「3問作れ」と書いても、小さいモデルは1問しか返さないことが多い。
   100パッセージ × 1問 = 100件
3. **フィルタが落とす** — 重なり不足・重複・長さ。特に `minAnswerChars` を大きくすると、
   「DC12Vです。」のような**正しい短い事実回答**がまとめて落ちる（既定を6にしてある理由）

対策はそれぞれ:

- **ページ単位のパッセージ** (`passageMode: 'page' | 'auto'`): OCR登録した資料は各チャンクに
  由来ページ (`doc.pages`) を持つので、同じページのチャンクを1パッセージにまとめる。
  300ページ → 300パッセージ。`pagesPerPassage` で数ページまとめることもできる
  （1ページが薄い資料向け）。ページ情報が無ければ自動でチャンク単位に落ちる
- **追い生成** (`topUp`): 1パッセージの採用数が `questionsPerPassage` に届かなければ、
  **既出の質問を見せて不足分だけ**作り直させる。同じ質問が返ってこないよう、
  生成プロンプトの末尾に既出リストと「別の箇所・別の観点から」の指示を足す
- **内訳の可視化**: 不採用を理由コード (deictic / meta / numbers / coverage / duplicate /
  length / verify) で数え、ジョブに出す。`finish_reason === 'length'` も数える
  （max_tokens 切れはJSONが途中で終わるので、拾える件数が静かに減る）

`dedupeThreshold` の既定を 0.92 に上げてあるのも同じ理由で、0.85 では
「OG-100の設定方法は？」と「OG-200の設定方法は？」のような1語違いまで重複として落ちる。

### 接続できないときの扱い（「fetch failed」対策）

Node の `fetch` は接続できないと一律 `TypeError: fetch failed` しか返さず、実際の理由は `e.cause.code`
に隠れている。そのまま画面に出すと「生成失敗: fetch failed」が数百行並ぶだけで、何を直せばいいのか
分からない。そこで3段構えにしてある:

1. **理由を訳す** (`describeFetchError`): `ECONNREFUSED` → 「llama-server が起動していないか、
   モデルがアンロードされています。チャットでモデルを読み込むか poolModel を設定してください」、
   `ECONNRESET` / socket hang up → 「VRAM不足で異常終了した可能性」、`ENOTFOUND` → 「ホスト名を
   解決できません」のように、次にやることまで書く
2. **接続系だけ数回リトライ**: プールのワーカーは起動直後にポートが開くまでの隙がある。
   接続系エラーだけ間を置いて2回まで再試行し、HTTPエラーやパース失敗は再試行しない（粘っても無駄なので）
3. **接続系はジョブごと止める**: 繋がらない状態で数百パッセージ回しても全部同じ理由で失敗する。
   1パッセージの失敗を記録して進む扱いから外し、ジョブを `failed` にして理由を1つ表示する。
   出力JSONLは残るので、原因を直せば「続きから」で再開できる（処理済みパッセージは飛ばす）

**根本の原因**は別にあって、既定構成 (`poolModel` 未設定) ではメインチャットの llama-server を
借りるのに、チャットは「リクエストが来たら起動」方式だという点。生成側が何もしなければ、
未ロードのポートを叩いて必ず `ECONNREFUSED` になる。さらに `chatLastUsed` はチャットの
プロキシ経由でしか更新されないため、**長いジョブの途中で `idleUnloadMs` のアイドル判定に
引っかかってモデルがアンロードされる**（生成が進むほど確実に踏む）。

そこでサーバー側で:

- `ragTuneEnsureMainChat()` … 未ロードなら `startChatModel()` でロード完了まで待つ
  （起動中なら `waitForReady`）。起動できるモデルが無ければ、その理由でジョブを止める
- `handle.keepAlive()` … 生成のたびに `chatLastUsed` を更新。アイドルアンロードに足元を抜かれない
- `/tuning/rag/status` … 未ロードでも自動起動できるなら `ok: true` + `note`（何が起動するか、
  チャットが待たされること）を返す。モデルのファイルが無い場合だけ `ok: false`

### API

| メソッド | パス | 用途 |
|---|---|---|
| GET | `/tuning/rag/status` | 機能ON/OFF・LLMの用意・登録ドキュメント数・**既定パラメータとプロンプト** |
| GET/POST | `/tuning/rag/jobs` | ジョブ一覧 / 作成（既定でそのまま開始） |
| GET | `/tuning/rag/jobs/:id` | 個別ジョブ |
| GET | `/tuning/rag/jobs/:id/stream` | 進捗のSSE（パッセージ1つごとに status イベント） |
| POST | `/tuning/rag/jobs/:id/start` | 開始 / 続きから再開（`{redo:true}` で最初から） |
| POST | `/tuning/rag/jobs/:id/cancel` | 中断（それまでの出力は残る） |
| DELETE | `/tuning/rag/jobs/:id` | ジョブ + 生成結果の削除 |
| GET | `/tuning/rag/jobs/:id/samples` | 生成結果の閲覧（`?status=accepted|rejected|all`） |
| POST | `/tuning/rag/jobs/:id/samples/status` | 採用 / 除外の手動切り替え |
| POST | `/tuning/rag/jobs/:id/import` | 学習サンプルDBへ取り込み（`{mode:'closed'|'open'|'both'}`） |
| GET | `/tuning/rag/jobs/:id/export` | 学習サンプル形式のJSONLダウンロード |

権限は既存のRAG系と同じ `ml:read` / `ml:write`（Cookieセッションは全権）。

### 運用の勘所

- **まず `maxPassages` を5〜10にして試運転する**。生成結果を見てプロンプトとしきい値を直してから全件回す。
  数百パッセージ回してから「質問が全部同じ形だった」と気づくのが一番高くつく
- **temperature は 0.3〜0.6**。低すぎると同じ聞き方ばかりになり、言い換えのバリエーションが出ない
- **不採用が多いときは理由を見る**: 「資料に無い数値」が多い → モデルが一般知識で補っている（プロンプトの禁止事項を強める）。
  「重なりが低い」ばかり → `minCoverage` が厳しすぎるか、パッセージが短すぎて回答が抽象的になっている
- **件数の目安**: 知識注入 (closed) は同じ事実を言い換えで3〜5通り × 対象事実の数。
  RAG運用の型を教える (open) だけなら数百件でも効く
- **生成に使うモデルは、学習するモデルより賢いものを使う**。教師データの質は生成モデルの上限を超えない

---

## 🧠 チャットの会話を教師データにする設計（人が介在する登録）

回答の「🧠 学習データに登録」から開くダイアログ (`ChatSampleDialog` / `public/js/index.jsx`)。
RAGからの自動生成とは逆の入口で、**実際にユーザーとモデルが交わした会話**を教師データにする。

### なぜワンクリック登録にしないのか

会話ログをそのまま教師データにすると、たいてい使い物にならない:

| 会話では成立するが、教師データとしては壊れているもの | 学習後に起きること |
|---|---|
| 「さっきのをもう少し詳しく」 | 学習時に残るのは質問文だけ。指す先が無い入力を覚え、脈絡なく詳細を語り出す |
| 回答の【S1】【S2】(出典キー) | 資料を渡していない場面でも【S1】と書く |
| 「資料によると〜」 | RAG無しでも資料の話をする |
| `[[gen_image:...]]` 等の画面用マーカー | 学習ノイズ。生成文に混ざる |
| そのターンだけ回答が間違っている / 脱線している | 間違いをそのまま覚える |

このうち機械で判別できるのは装飾の除去くらいで、**「この回答は正しいか」「この質問は単体で意味が通るか」は人にしか判断できない**。
そこで「取り込む前に人が直す」前提のダイアログにし、機械は掃除と警告だけを担当する。
（RAGからの自動生成側は逆に量が主役なので、機械フィルタ＋LLM検証で自動的に落とす設計にしてある。
入口ごとに、人が見る前提か機械が捌く前提かを分けている。）

### 3つの登録モード

| モード | 作るもの | 使いどころ |
|---|---|---|
| ✏️ この1往復 | `{system, instruction, response}` 1件 | 文面を直して入れる基本形。前後のターンへも移動できる |
| 🔁 マルチターン | `{messages:[...]}` 1件 | 会話の流れ（前提を引き継いだやり取り）ごと覚えさせたいとき |
| ☑️ 会話から選ぶ | 選択したターンぶんの単一ターン | 良い往復が何度もあった会話を一気に取り込む。✏️ で個別編集へ |

登録APIは tuning.html と同じものを使う（サーバー側の追加実装なし）:

- 単一 → `POST /tuning/samples`
- マルチターン / 一括 → `POST /tuning/samples/import` (`format: 'jsonl'`)

マルチターンは `messages` 配列をそのまま送る。importer 側が「最後が assistant」を要求するので、
クライアントで **user 始まり / assistant 終わり** に揃えてから送る（途中で切り出すと末尾が user になり得る）。

### 自動クリーンアップと警告

`cleanSampleText()` が編集欄に入れる前に落とすもの: `<think>…</think>`、`[[gen_image:…]]` /
`[[gen_audio:…]]`、出典キー【S1】（チェックで残せる）、連続する空行。**元の会話は書き換えない**
（表示中のメッセージは触らず、ダイアログ内のコピーだけを加工する）。

`checkSampleQuality()` の警告は**登録を止めない**。止めると「意図的な拒否例」「あえて短い定型回答」
のような正当なケースまで入れられなくなる。判断材料だけ出して、決めるのは人という分担にしている。

`【S1】` の判定に `/g` 付き正規表現を `test()` で使うと `lastIndex` が進んで**呼ぶたびに結果が変わる**
（React の再レンダーで警告が点滅する）。判定用は非グローバル、置換用は `/g` と分けてある。

### RAG運用型 (open) での登録

永続RAGを使った回答には `msg.ragSources`（`{key, filename, label, pageText, text}`）が付いている。
「参考資料を入力に含める」をONにすると、`renderRagContext()` が
チャットの検索結果と同じ体裁（`── 資料 S1 ──` / `出典キー: S1` / 末尾に【S1】）で入力を組み立てる。

体裁とsystemは `config.json` の `tuning.ragDataset`（contextTemplate / openSystemPrompt / citationKey）を
**RAGからの自動生成と共用**する。2箇所で別々のテンプレートを持つと、片方を直したときにもう片方だけ
推論時の形とズレる。資料が2件以上のターンはテンプレートでは表現できないので、その場合だけ
S1…Sn のブロックをクライアントで組む。

### 状態の扱い

- 編集欄の初期値は `useState` の初期化関数で入れる（`useEffect` 任せだとマウント直後に空欄が一瞬見える）
- ターンを移動すると編集内容は失われるので、既定値と違えば `confirm()` で確認する
- タグは `chat` + `closed` / `open` / `multiturn` を自動付与。後から学習データを絞り込める
...
```
  ...
  rl_runner.py (subprocess)            rl_online_server.py (常駐ワーカー :11600)
   ...
   ※ オフラインとオンラインで前処理・損失の挙動を一致させる (乖離防止)
                       ...
                       │ 状態ベクトルの後半 (視覚を使う場合のみ)
              vjepa2_common.py
   クリップ読込 / 凍結 V-JEPA 2 / プーリング / 埋め込みキャッシュ
```
...
| 関数 | 役割 |
|---|---|
| `build_qnet(torch, nn, state_dim, out_dim, hidden, emb_dim=0)` | 3層 MLP の方策/Qネットワーク (Linear→ReLU→Linear→ReLU→Linear)。オフライン/オンライン同一構造。`emb_dim>0` なら状態ベクトル末尾 `emb_dim` 次元に `LayerNorm` を通してから MLP へ |
| `encode_state_dict(state, meta, np)` | 状態辞書 → 数値ベクトル。カテゴリ列は index 化 (未知値→0)、数値列は float 化し、`(vec - mean) / scale` で正規化。`meta['embDim']>0` なら末尾に `state['_embedding']` を素のまま連結 |
| `bc_loss(qnet, nn, S, A, action_spec, mask)` | BC の損失。離散はチャンク位置ごとの cross entropy、連続は Huber (smooth L1)。`mask` でエピソード終端のはみ出し位置を除外 |
| `compute_loss(algo, mode, ...)` | 1ミニバッチの損失。DQN/DDQN は Bellman 誤差 (smooth L1)、CQL は + 保守正則化項、BC は `bc_loss` に委譲 |

**方策ヘッドの出力次元 (`out_dim`)** は行動仕様で決まる:

| 構成 | out_dim | 出力の解釈 |
|---|---|---|
| 離散 (従来) | クラス数 N | Q値 / ロジット |
| 離散 + チャンク K | K × N | `(K, N)` に view して位置ごとに argmax |
| 連続 Da 次元 + チャンク K | K × Da | `(K, Da)` に view。正規化空間の行動ベクトル |

`action_spec` を省略すると「離散・チャンク1」として扱うので、**この変更以前に学習したモデルも
オンラインワーカーもそのまま動く** (`out_dim == n_actions` で従来と同一の state_dict)。
...
- **`emb_dim == 0` のとき `emb_norm` は生成しない**ので `state_dict` のキーは従来と完全に同一。
  この変更以前に学習した `model.pt` はそのまま読める。

### 👁 視覚観測 (`vjepa2_common.py`)

観測 (動画 / フレーム連番ディレクトリ / 静止画) を Meta の V-JEPA 2 で埋め込みに変換し、
状態ベクトルに連結する層。**エンコーダは常に凍結**し、勾配は方策ヘッドにしか流さない。

```
state = [ StandardScaler(表の列)  ‖  V-JEPA 2 埋め込み(1024次元) ]
          └ scalerMean/Scale で正規化      └ 正規化は方策ヘッドの LayerNorm が担当
```
...
| 判断 | 理由 |
|---|---|
| 埋め込みに StandardScaler / Welford を掛けない | 1024次元に少数サンプルの分散推定を当てると scale≈0 の次元が出て発散する。LayerNorm ならサンプル数に依存しない |
| 表の列と**連結**する (置き換えない) | `stateColumns` の仕組みをそのまま使えるので、DQN/DDQN/CQL/BC の全アルゴリズムが無改造で視覚対応になる。関節角などの proprioception も同時に入れられる |
| 埋め込みをディスクにキャッシュ | エンコードは方策ヘッドの学習より2〜3桁遅い。ハイパラを変えて再学習するたびに再エンコードすると開発が回らない |
| キャッシュキーに指紋を使う | 内容の SHA1 は動画では高すぎるので `(絶対パス + サイズ + mtime)` で代用。ディレクトリはファイル数と最大 mtime を混ぜる |
| キャッシュを float16 で保存し、**初回も fp16 に丸めてから返す** | そうしないと「キャッシュを作った回だけ fp32、次回以降は fp16」という微妙な不一致が出る |
| オフラインは使い捨てプロセス | 学習前に `Encoder.close()` + `empty_cache()` で VRAM を返す (llama-server との競合回避)。常駐エンコーダは Phase 3 |
| パスは uploads 配下に固定 | テーブルの中身はユーザーデータなので、`resolve_source` が `realpath` で基点外を拒否する |

- **プーリング**: V-JEPA 2 の出力は `(B, seq_len, hidden)` で、`seq_len = (T/tubelet) × (H/patch) × (W/patch)`。
  `mean` は全トークン平均、`mean_last` は最後の時間スライスのトークンのみ平均 (直近の観測を重視する制御向け)。
  トークン格子が想定と合わない場合は `mean` にフォールバックする。
- **クリップのサンプリング**: 末尾を最新フレームに合わせて `frames` 本を `stride` 間隔で取る。
  制御では「直近の過去」が効くため、先頭からではなく末尾から数える。総フレーム数が足りない場合は
  先頭フレームを繰り返してクランプする。
- **エンコード設定は学習時のものを `config.json` に保存**し、評価・推論では必ずそれを使う
  (`_embed_cfg_for_model`)。ここがズレると埋め込み空間が変わり、方策が**エラーを出さずに**壊れる。
- **動画デコーダ**: torchcodec → `torchvision.io.read_video` (PyAV) → decord の順にフォールバック。

### 🤖 Behavior Cloning の拡張

デモの模倣を実用レベルにするための3点。いずれも `algo=bc` 専用 (連続値やチャンクは argmax
できないため、価値ベースのアルゴリズムでは成立しない)。

**1. 連続行動 (回帰)**

`actionType=continuous` + `actionColumns` で、行動を数値ベクトルとして Huber 損失で回帰する。

- **行動を正規化してから学習する**。関節角とグリッパ開度のようにスケールが桁違いの列が混ざると、
  Huber がスケールの大きい次元に引っ張られて小さい次元を無視するため。
  `actionMean` / `actionScale` を config に保存し、推論時に元スケールへ戻す。
- 正規化の統計は **学習側の行だけ** から取る (検証行を混ぜると評価が甘くなる)。
- 評価は一致率ではなく MAE / RMSE。**ログ行動の標準偏差との比**も併記する
  (比が 1 に近い = 「平均を出しているだけ」で模倣できていない、という判断ができる)。

**2. 行動チャンキング**

状態1件から K ステップ先までの行動をまとめて予測する (ACT / π0 系の定番)。BC の
「1手ずつ予測して誤差が積み上がる」弱点が緩和され、動きが滑らかになる。

```
状態 s_t  →  ヘッド  →  [a_t, a_{t+1}, ..., a_{t+K-1}]
                          └ 実行するのは先頭だけでも、K手まとめてでもよい
```

- **エピソード列が必須**。境界をまたいで「次のデモの最初の行動」を教師にしてしまうため。
- エピソード終端では K 本に満たないので、`_chunk_targets` が返すマスクで
  はみ出した位置の損失を 0 にする。マスクは単調 (一度切れたら以降も 0)。
- チャンク作成の前に `(episodeColumn, stepColumn)` で並べ替える。`stepColumn` 省略時は
  テーブルの行順を時間順とみなす (警告を出す)。

**3. 検証分割 (`valSplit`)**

| 判断 | 理由 |
|---|---|
| **エピソード単位**で分割する (エピソード列がある場合) | 行単位のランダム分割だと、隣接フレームが学習側と検証側に分かれる。観測がほぼ同一なので検証値が実力より大幅に良く出る |
| scaler / 行動正規化を **学習側の行だけ** で当てる | 検証行の統計が前処理に混ざると、その分だけ検証値が甘くなる |
| エピソード列が無い場合は行単位 + **警告ログ** | 使えないよりはマシだが、値を鵜呑みにしてはいけないと明示する |
| `policyAgreement` は検証があれば検証値を入れる | 既存フィールドの意味は変えるが、`valSplit>0` は新機能なので既存の学習結果には影響しない。`trainAgreement` / `valAgreement` も別に保存 |
| 学習/検証の差が 15pt を超えたら警告 | 過学習の一番分かりやすいサイン |

学習曲線には検証損失を点線で重ねて描く。検証が下げ止まって上に転じた地点から先が過学習。

**報酬列は BC では任意**。デモ動画には報酬が無いことが多いため、`rewardColumn` を省略すると
報酬 0 で埋める (BC は報酬を参照しない)。BC 以外では従来どおり必須。

**オンライン学習は離散・チャンク1のみ**。連続行動とチャンキングは `act` の argmax と
経験再生がそのままでは成立しないので、`load_offline_agent` が明示的に断る
(誤った形のネットを黙って組み立てるより良い)。


### ⚡ 常駐エンコーダ (`vjepa2_server.py`)

オフライン学習では「1ジョブで数百クリップ」をまとめてエンコードするので使い捨てプロセスで
十分だが、推論は「1状態を何度も」なので、そのたびに 1.3GB のモデルを読み直すと話にならない。

```
                        server.js
   /policy ────┐   /act ────┐   /learn ────┐
               ▼            ▼              ▼
        attachEmbedding / attachEmbeddingsBatch
               │  観測(パス or 生フレーム) → 埋め込み
               ▼
        vjepa2_server.py (常駐 :11601)      ← VRAM にモデル常駐
          /embed        パス + ディスクキャッシュ
          /embed_frames base64フレーム (キャッシュしない)
               │
               ▼  state._embedding として注入
        rl_runner.py /policy  または  rl_online_server.py /act
          (小さな方策ヘッドを読むだけ)
```

| 判断 | 理由 |
|---|---|
| 学習は使い捨て、推論は常駐 | 学習ジョブは終了時に VRAM を返すべきで、推論は再ロードしたくない。要求が逆なので分ける |
| アイドル自動解放を既定 10分 | llama-server / sd-server と VRAM を取り合う。rl-online (既定 0 = 常駐) より積極的に落とす |
| エンコーダ保持数を既定1 | spec が違えば別モデルになる。VRAM を守るため LRU で1つに制限 |
| `/embed` はキャッシュあり、`/embed_frames` は無し | パスは同じものが繰り返し来る (再学習・再評価)。生フレームは毎回違うのでキャッシュは無駄 |
| 推論そのものは `INFER_LOCK` で直列化 | 同一モデルへの並列 forward で VRAM が跳ねるのを防ぐ |
| `/policy` はワーカー障害時に従来経路へフォールバック | エンコーダが起動できないだけで推論が全部止まるのは割に合わない。遅くても動く方を選ぶ |
| `learn` のバッチはパスを1回の `/embed` に束ねる | 経験をまとめて投入しても往復とエンコードが1回で済む |

- 生フレームの受け口 (`frames: ["<base64 PNG>", ...]`) があるので、**ロボットやゲームは
  ファイルを書かずに今の画像をそのまま投げられる**。1枚だけ渡せば静止画として複製する。
  フレームの選び方 (末尾から stride 間隔) は学習時と同一のロジックを共有している。

### 🎬 経験ログ用データテーブル

表データ機能の取り込み口 (`/ml/datasets/import/csv`) は「手元にCSVがある」前提だが、
強化学習のデータは **これから溜める** ことの方が多い。収集の系統を別に用意する。

```
ロボット / ゲーム / シミュレータ
        │  POST /ml/rl/datasets/record
        │  { table, episode, steps: [{ state, frames[], action, reward, done }] }
        ▼
   server.js
     ├─ frames(base64) → uploads/rl_datasets/<表>/<エピソード>/<step>.png
     │                    (複数枚ならディレクトリ = フレーム連番)
     └─ 行を組み立てて DuckDB へ追記 (step はエピソードごとに自動採番)
        ▼
   そのまま「➕ エージェントを新規作成」で学習
```

| 判断 | 理由 |
|---|---|
| 観測を記録と**同じリクエスト**で受ける | 別々だと「ファイルは上がったが行が無い」等の不整合が起きる。1往復で完結させる |
| `step` はサーバ側で自動採番 | クライアントに連番管理をさせない。既存エピソードの `MAX(step)+1` から続ける |
| エピソードIDを厳格な allowlist で検証 | そのままディレクトリ名になるため。`[A-Za-z0-9_.-]` のみ |
| 画像はマジックバイトで形式判定 | 拡張子やContent-Typeの申告を信じない。PNG/JPEG/WebP 以外は書かない |
| `framePath` は uploads 配下に限定 | 既にサーバ上にあるファイルを指す場合も、外を参照させない |
| 学習中 (`currentRlJob`) は記録を 409 で拒否 | Python 側が DuckDB を read_only で握っており、Node 側の接続は解放済みのため |
| `INSERT INTO ... BY NAME` を使う | 報酬なしのステップなど、列が欠けた行でも位置ズレせず入る |
| エピソード単位の削除を用意 | デモ収集では失敗した回が必ず出る。行と観測ファイルをまとめて捨てられないと運用が回らない |

観測ファイルの削除は `uploads/rl_datasets/` 配下、つまり **このエンドポイントが自分で
作った場所** に限る。`framePath` で外部から参照させたファイルは消さない。

### 🌍 世界モデル (V-JEPA 2-AC + CEM 計画)

BC が「デモの真似」なのに対し、世界モデルは「行動で潜在がどう変わるか」を学び、
推論時は方策を使わず **CEM で行動系列を探索** する。

```
学習 (vjepa2_ac_runner.py):
  経験ログ (episode/step/frame/action)
    → 凍結 V-JEPA 2 で潜在化 (キャッシュ共用)
    → predictor: (z_t, a_t) → z_{t+1}   [残差MLP・Δz学習・出力ゼロ初期化]
    → rolloutK ステップの自己回帰で損失 (計画時の多段予測に耐えるため)

計画 (vjepa2_server.py /plan):
  現在の観測 + ゴール画像 → エンコード (GPU)
    → CEM: 候補系列を サンプル → 潜在ロールアウトで採点 → 上位で分布更新 (CPU)
    → 行動系列を返す。MPC は先頭1手だけ実行して再計画
```

| 判断 | 理由 |
|---|---|
| predictor は残差MLP (数百万パラメータ)、論文の 300M Transformer ではない | 対象がプーリング済み潜在で、データが数百〜数万遷移の規模。300M は過学習するだけ |
| Δz を学び、出力層をゼロ初期化 | 初期状態が恒等写像になり、学習が安定。恒等ベースラインとの比較もそのまま成立 |
| 学習損失は rolloutK ステップの自己回帰 | 1ステップだけ学習すると、計画時の多段ロールアウトで誤差が爆発する |
| **改善比** (恒等÷モデル) と **行動感度** (シャッフル÷本来) を必ず出す | 潜在MSEの絶対値は解釈できない。「恒等より良いか」「行動を使っているか」が計画可否の全て。UI はどちらかが 1.1x 未満なら警告 |
| 遷移は step が +1 連続の行だけから作る (`build_windows`) | エピソード削除や欠落で飛んだ箇所を1ステップ遷移として学ぶと力学が汚れる |
| CEM の分布更新は numpy 純関数、ロールアウトは注入 | torch 無しで動作を検証できる |
| 計画は常駐エンコーダに同居、ロールアウトは CPU | エンコーダの VRAM を使い回し、predictor は CPU で十分速い。model.pt の mtime で再学習を自動検知 |
| 学習ジョブは RL 学習とスロット共有 (`currentRlJob`) | DuckDB を read_only で開く外部プロセスは同時に1つ、という既存の排他を守る |

**用途による一覧の分離.** 教師あり学習の表と強化学習の経験ログは、列の意味も使う画面も
違うので、同じ一覧に混ぜると選び間違える。ただし DuckDB を分けるのは大がかりなので
(`ML_DB_FILE` は release/reacquire の調停を含めて広く使われている)、
**同じ DB に置いたまま印で分ける** 方針を取った。

| 判断 | 理由 |
|---|---|
| `ml_meta.json` の `kind` ('ml' / 'rl') で分ける | DBを分けると学習ジョブの排他制御まで作り直しになる。一覧の分離が目的なら印で足りる |
| `kind` 未設定は `rl` 定義の有無で推定 → 無ければ `ml` | 既存テーブルの見え方を変えない。移行作業を不要にする |
| `GET /ml/datasets` の既定は全件 | 外部API・LLMツールの既存の呼び出しを壊さない。UI だけが `?kind=` を明示する |
| 用途は後から変更できる (`PUT /ml/datasets/:name/kind`) | CSV で取り込んだ経験ログを RL 側へ回す導線が要る。片方向だと詰む |
| 学習ダイアログに「両方表示」のチェックを残す | 分離は既定の絞り込みであって、選べなくする壁ではない |
...
2. DuckDB テーブルから 状態/行動/報酬/次状態/done 列を読み、経験再生バッファを構築。
   `videoColumn` があれば、ここで観測を V-JEPA 2 の埋め込みに変換して状態に連結する
   (現状と次状態のパスをまとめて1回のエンコーダロードで処理し、終わったら VRAM を返す)。
...
| 固定する時は **デバイス選択系の引数を落とす** (`--device` / `--tensor-split` / `--main-gpu` / `--split-mode`) | これらは「絞る前のデバイス一覧」を前提にしている。`commonArgs` に `--device ROCm0,ROCm1` があるまま1枚だけ見せると、見えている1枚は `ROCm0` と名乗るので **`invalid device: ROCm1` で起動に失敗する**（実際にこれで落ちた）。1枚しか見えない状態ではそもそもデバイス選択は不要 |
| `--device` の指定は **許可リストとして尊重する** | 「このGPUは学習用に空けておく」といった意図で絞っている環境がある。空きが多いからと許可外のGPUに載せてはいけない。許可されたGPUに収まらなければ固定せず分散に戻す（`--device none` は「オフロードしない」の意味なので固定対象外） |
