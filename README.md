...
- **`compaction`（デフォルト、Claude風）**: 履歴は無圧縮のまま送信し、コンテキスト使用率が閾値（`contextCompaction.threshold`、既定75%）を超えた時だけ、LLM自身に古い会話を要約させて1つの要約メッセージに置き換えます。トークン使用率は圧縮が走るまで単調に増え、圧縮のタイミングで一段下がります。圧縮位置はチャット上に「📦 コンテキスト圧縮」の区切りとして表示され、クリックで要約を確認できます。
  ...
  要約の生成は堅牢化されています: 思考型モデル（Qwen3系）が思考だけで要約予算を使い切って「要約結果が空でした」になるのを防ぐため、**要約呼び出しでは思考を無効化**（`chat_template_kwargs.enable_thinking=false`）し、それでも空なら `max_tokens` を3倍にして1回だけ自動で引き直します。また、圧縮対象の会話が大きく**要約リクエスト自体がコンテキストに収まらない場合は自動でチャンクに分割**し、「前回要約＋続き→統合要約」の形で順次要約します。
...
また、**思考だけで最大出力トークンを使い切り、回答本文に到達しないまま終わった場合** (出典キーの検討などの思考が長引き「思考中で停止した」ように見えるケース) は、思考を無効化 (`chat_template_kwargs.enable_thinking=false`) して**回答のみを自動で1回引き直します**。思考が `<think>` タグや `reasoning_content` ではなく本文側に英語のまま漏れて途中で切れた場合も、同じ引き直しで復旧します。
...
### ⏳ 生成中の離脱ガード
思考・回答の途中でチャットを切り替えたりページを離れようとすると、確認ダイアログが出る。
「移動する」を選ぶと、進行中の生成を中断してから移動する。

| 操作 | 出るダイアログ |
|:--|:--|
| チャット履歴のクリック / 「+ 新規」 / チャット削除 | アプリ側の確認（日本語） |
| ブラウザの戻る・進む | アプリ側の確認。中止するとURLを押し戻して元のチャットに留まる |
| ファインチューニング・機械学習・OCR・設定へのリンク | アプリ側の確認 |
| リロード・タブを閉じる・アドレスバーからの移動 | ブラウザ標準の確認（`beforeunload`。文言はブラウザ固定で変更不可） |

ガードが無いと、進行中のストリームが**切り替えた後のチャットに書き込まれる**。
生成中は自動保存を止めているため、生成が終わった瞬間に別のチャットの続きとして
まとめて保存されてしまい、後から見ると回答が紛れ込んだ状態になる。
...
新規チャット画面で「LLMに役割を与える」ボタンから、フリーテキストで役割や指示を設定できる。例：「あなたは熟練のPythonエンジニアです」「小学生向けに優しく説明してください」「回答は3行以内で関西弁」など。設定した役割は内部のシステムプロンプトに追加されてLLMの応答スタイル・専門性・形式を大きく変える。チャット中はヘッダー🎭アイコンからモーダルで再編集可能。チャット履歴と一緒に保存され、過去のチャットを開くと役割も自動復元される。
...
- **LLM チャット連携**: `ml_list_datasets` / `ml_describe_dataset` / `ml_query_dataset` / `ml_list_models` / `ml_predict` に加え、書き込み系の `ml_import_csv` / `tuning_import_samples` の計7ツールで、LLM が自律的に「データ確認 → 集計 → 予測 → 結果説明」や「生成CSVの登録」を実行
...
- **LLM チャット連携**: 画像を添付して「何が写ってる?」と聞くと、LLM が `detect_objects` ツールを呼んで物体を検出し、結果を解釈して回答 (Vision対応モデルなら誤認識の訂正も)。「〇〇モデルで検出して」とモデル名を明示すれば `model` 引数で**学習済みカスタムモデルでの検出**も可能。`detect_keypoints` (キーポイント検出、COCO人体17点/カスタム) と `image_list_models` (学習済みカスタムモデル一覧) も利用できる。データセットの作成・画像追加・アノテーション・学習開始は現時点ではチャット非対応 (画像学習のデータセット一覧にもその旨を表示)
...
| `math` | 数式を KaTeX 用の LaTeX（`$ ... $` / `$$ ... $$`）で書かせる指示（常時。空文字で無効） | - |
| `meta` | メタ抑制指示（常時） | - |
| `judge` | ツール判断専用（軽量） | `{toolList}` |
| `rag` | サーバー登録ドキュメント（RAG）の引用ルール。原文どおりの数式・記号の書き写し、出典キー `【S1】` の付与、記憶に頼った引用の禁止など | - |

#### 数式を LaTeX で書かせる（`math`）

画面は `$ ... $` / `$$ ... $$` を KaTeX で描画しますが、**指示が無いとモデルは数式を地の文のまま書きます**（`pm = W / (2BD)`、`p0(X) = k1 {s0(X)}^n1` のように添字も分数も潰れた形）。読みにくいだけでなく、**数式の照合バッジ（`✓ 資料と一致`）も出ません** — 照合は数式区切りのある箇所しか拾わないため、数式0本と見なされて素通しになります。

`math` はこれを防ぐための常時付加セクションで、「文中は `$ ... $`、独立した式は `$$ ... $$`」「添字は `_`、2文字以上は `{ }`」「数式内に日本語を書かない」といった規則と、`pm = W / (2BD)` → `$$p_m = \frac{W}{2BD}$$` の対比例を持ちます。トークンを節約したい場合は `""`（空文字）にすればセクションごと付きません。

**1つの `$$ ... $$` に入れるのは式1本だけ**、というのも重要な規則です。`$$0 \leq s_0(X) \leq H \text{ のとき, } \quad p_0(X) = k_1 \{ s_0(X) \}^{n_1}$$` のように条件と式を1本に詰め込むと、式単位で載っている資料のどこにも同じ文字列が無くなり、正しい引用なのに `⚠ 資料と不一致` と表示されます。条件は数式の外の地の文に書かせます。照合側でも `\text{…}` を中身ごと、`_` と `^` を記号として無視するようにしてあるので、資料の OCR が `n1`、回答が `n_1` でも一致と判定されます（記号の並び順は残るため、`k_1 s_0` と `k_0 s_1` の取り違えは今までどおり検出されます）。

永続RAGを併用する場合は `rag` 側の「原文を1文字も変えずに写す」と衝突しないよう、**変えてよいのは LaTeX の体裁（区切り記号と添字の書き方）だけで、記号・添字・係数・項の並びは原文どおり**、と明示してあります。検索結果がすでに `$ ... $` なら区切りごとそのまま写し、OCR が LaTeX 化できず平文になっている箇所は「読み取れた記号のまま `$ ... $` で囲むだけ」（落ちた分数の横線を推測で補わない）という切り分けです。
...
チャットで生成した CSV/TSV を、コピー&ペーストなしでそのまま学習用データとして登録できる。入口は2つ:

### ① コードブロックのボタンから (UI)

LLM が ` ```csv ` / ` ```tsv ` コードブロックを応答に含めると、ヘッダーに **「📊 学習に登録」** ボタンが表示される。クリックすると登録ダイアログが開く:

- **プレビュー**: 列数・行数と先頭5行を表示
- **🗂️ MLデータテーブル (機械学習)**: テーブル名・説明・モード (置換/追記) を指定して DuckDB に取り込み (`POST /ml/datasets/import/csv`)。登録後は 🤖 機械学習ページの「モデル学習 > データテーブル」に表示され、モデル学習・SQL分析に使える
- **🧠 ファインチューニング教師データ**: ヘッダーに `instruction` と `response` 列 (任意で `system`) がある場合に選択でき、教師データとして**追加**登録 (`POST /tuning/samples/import`)。既存データは消えない。列が無い場合は選択不可の理由を表示

### ② チャットの指示から (LLMツール)

「このCSVをMLに登録して」「教師データに追加して」のように指示すると、LLM がツールで登録する:

- `ml_import_csv(tableName, mode?, description?, csvContent?)` … データテーブルへ取り込み。**既定は追記 (append)** で、置換 (replace) はユーザーが「上書き」「置き換え」と明示した時のみ
- `tuning_import_samples(csvContent?)` … instruction/response 列のCSVを教師データに追加

登録するCSVは、`csvContent` を省略すると**会話の直近の ` ```csv ` / ` ```tsv ` コードブロックが自動で使われる** (クライアント側で解決)。LLM がCSV全文をツール引数に再掲するとトークン上限で呼び出しJSONが壊れるため、ツール定義でも省略を必須級に指示している。

どちらも書き込み系ツールとしてエージェントハーネスの権限モードに従う (`plan` では拒否、`default` では許可リスト外なら確認、既定の `bypassPermissions` では許可)。ML系ツールと同じプリフィルタ (メタキーワード) の対象。
...
「🗂️ データテーブル」「🔍 SQLクエリ」「🧠 モデル学習」の 3 つのサブタブで構成され、学習データの管理・SQL でのデータ確認からモデルの学習・予測までをこのタブ内で完結できる。
...
- テーブル一覧 + クリックで詳細 (スキーマ、プレビュー50行、取得元URL)。「🔍 SQLで開く」で SQLクエリサブタブへ移動
- 🗑️ テーブル削除
...
`ml.enabled: true` でチャット画面に7つのMLツールが自動追加される:
...
| `ml_import_csv(tableName, mode?, description?, csvContent?)` | 会話で生成したCSVをデータテーブルに登録 (既定は追記。置換はユーザーが明示した時のみ) |
| `tuning_import_samples(csvContent?)` | instruction/response 列のCSVをファインチューニング教師データに追加登録 |

書き込み系2ツールの `csvContent` は**省略推奨**: 省略すると会話の直近の ` ```csv ` / ` ```tsv ` コードブロックをクライアント側で自動参照する。ツール判断のLLM呼び出しは小さい `max_tokens` で走るため、LLM にCSV全文を引数へ再掲させると途中で切れて llama-server のツール呼び出しJSONパースが失敗する (「missing closing quote」の 500)。参照方式なら引数はテーブル名程度で済み、この失敗が起きない。

このほか**画像学習系のチャットツール** (すべて読み取り専用):

| ツール | 用途 | 提供条件 |
|:--|:--|:--|
| `detect_objects(threshold?, model?)` | 添付画像の物体検出。`model` で学習済みカスタムモデルを指定可 (省略時はCOCO 80クラス) | 画像添付時 |
| `detect_keypoints(threshold?, model?)` | 添付画像のキーポイント検出 (省略時はCOCO人体17点、`model` でカスタムモデル) | 画像添付時 |
| `image_list_models()` | 画像学習タブで学習したカスタムモデル一覧 (検出クラス名・キーポイント名付き) | 画像添付時、または「画像モデル」「検出モデル」等の発話時 |

データセットの作成・画像追加・アノテーション・学習開始などの**書き込みは現時点ではチャット非対応** (誤爆時のGPU長時間占有リスクと、チャットからの画像一括アップロードが実用的でないため)。画像学習のデータセット一覧の下部にもその旨を表示している。

これらのツールは、ユーザー発話に**具体的なテーブル名/モデル名**または**MLメタキーワード** (「ML」「機械学習」「データテーブル」「予測して」「登録して」等) が含まれている時のみ自動的に有効化される。雑談やコード生成では呼ばれないので、LLM の応答品質を保ちつつ安全に共存できる。書き込み系の `ml_import_csv` / `tuning_import_samples` はエージェントハーネスの権限モードでも書き込みツールとして扱われる (`default` モードでは許可リスト外なら確認、`plan` モードでは拒否)。
