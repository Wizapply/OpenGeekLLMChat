...
検索でヒットしたチャンクの本文は `ragSources[].text` として画面側に残るので、回答の数式（`$...$` / `$$...$$` / `\(...\)` / `\[...\]`）がそこから写されたものかを、モデルに聞かずに文字列比較で確かめます。空白・波括弧・`\left`/`\right`/`\cdot` の差は正規化して吸収します。短すぎる式は何にでも一致してしまうため、**インラインは正規化後25文字以上、ブロック数式（`$$…$$`）は10文字以上**を判定対象にします。ブロック数式は「これが式です」という主張なので、`$$p_m = \frac{W}{2BD}$$`（正規化13文字）のような短いものも見ます。
