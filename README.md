...
- **後処理パイプライン**: 学習完了後にUIから「📦 マージ→GGUF→量子化」を1ボタン実行。実行中はジョブに **「📦 マージ実行中（Step 1: マージ / Step 2: GGUF変換 / Step 3: 量子化）」** と経過時間が表示され、後処理ログが3秒ごとに更新される（停止ボタンあり）。失敗・中断もその場に残る
...
- **素材は永続RAGのチャンク**: 検索用チャンク（既定500文字）を束ね直して「パッセージ」にする。Q&Aを作るには500文字では文脈が足りないため（定義と条件が別チャンクに割れる）
- **ページ単位で作れる**: PDF OCR で登録した資料はページ番号を持っているので、`passageMode: "page"`（既定の `auto` でも自動選択）にすると **1ページ = 1パッセージ**。「1ページにつき3問」なら300ページで最大900問になる。ページ情報が無い資料は従来どおりチャンク単位
- **足りなければ追い生成**: 小さいモデルは「3問作れ」と言っても1問しか返さないことが多い。指定数に届かなければ、既出の質問を見せて不足分だけ作り直させる（`topUp` / `topUpRounds`）
- **件数が伸びない原因が見える**: ジョブに**不採用の内訳**（質問が指示語 / 資料に無い数値 / 重なり不足 / 重複 / 長さ / 検証NG）と、`max_tokens` で応答が切れた回数を表示する
...
> **思ったより件数が少ないとき**: ①「パッセージの作り方」を**ページ単位**にする（チャンク単位だと数ページ分が1パッセージにまとまり、設問数がページ数より減ります）②「不足分を追い生成する」をONにする ③ ジョブの**不採用の内訳**を見る — 「資料との重なり不足」が多いなら `minCoverage` を下げる、「長さ超過/不足」が多いなら回答の最小文字数を下げる（日本語の事実回答は「DC12Vです。」のように短い）、「応答が max_tokens で切れた」が出ているなら `maxTokens` を上げる。

> **「生成LLMを確保できません: チャットモデル起動タイムアウト」と出たら**: 20GB級のモデルは初回ロードが長く、チャット用の `llamaServer.readyTimeoutMs`（既定120秒）では足りません。生成ジョブは `tuning.ragDataset.modelLoadTimeoutSec`（既定900秒）まで待つようになっていますが、それでも足りなければこの値を増やすか、先にチャット画面でモデルを読み込んでから生成を開始してください。大きいモデルは生成自体も遅いので、`tuning.ragDataset.poolModel` に軽めのモデル（7〜8B級）を指定して生成専用にするのが実用的です。
...
- **👁 V-JEPA 2 で観測から学習 (視覚 + Behavior Cloning)**: 学習画面で「観測カラム」に**動画 / フレーム連番ディレクトリ / 静止画のパス列**を指定すると、Meta の [V-JEPA 2](https://ai.meta.com/blog/v-jepa-2-world-model-benchmarks/) を**凍結した特徴抽出器**として使い、観測を埋め込みベクトルに変換して状態に連結する (`vjepa2_common.py`)。
  - 状態は `[ scaler(表の列) ‖ V-JEPA 2 埋め込み ]` の連結。表の列を0本にすれば**視覚のみ**の方策も学習できる。埋め込み側は scaler ではなく方策ヘッドの `LayerNorm` で正規化する (高次元に実行統計を当てると発散するため)。
  - **アルゴリズムに BC を選べば、デモ動画の模倣学習**になる。DQN / DDQN / CQL も同じ状態表現でそのまま動く (次観測カラムを指定すれば遷移ベース)。
...
- **🤖 Behavior Cloning の拡張 (連続行動・行動チャンキング・検証分割)**: デモの模倣を実用レベルにするためのオプション群。いずれも `algo=bc` のときだけ使えます。
  ...
  - **検証分割 (`valSplit`)**: 学習データの一部を取り分けて汎化性能を測ります。**学習データ上の一致率は必ず高く出る**ので、これが無いと過学習に気づけません。エピソードカラムがあれば**エピソード単位**で分割します（行単位だと隣接フレームが学習側と検証側に分かれ、検証値が実力より良く出るため）。学習曲線には検証損失が点線で重なり、学習/検証の乖離がそのまま見えます。
  - **報酬カラムは省略可**: BC は報酬を使わないので、デモ動画のように報酬列が無いデータでもそのまま学習できます。
  - 連続行動と行動チャンキングは**オンライン学習には未対応**です（オフライン学習と `/policy` 推論は使えます）。
- **⚡ 常駐エンコーダ（リアルタイム推論）**: `vjepa2_server.py` が V-JEPA 2 を VRAM に載せたまま常駐し、観測→埋め込みの変換を localhost HTTP で受けます。llama-server / sd-server と同じく**遅延起動・アイドル自動解放**（既定10分）。
  - これがないと推論のたびに 1.3GB のモデルを読み直すため、`/policy` 1回に30秒以上かかっていました。常駐後は数十ミリ秒〜です。
  - **生フレームを直接送れます**: `state` に `frames: ["<base64 PNG>", ...]` を入れると、ファイルに保存せずその場でエンコードします。ロボットやゲームから今のカメラ画像を投げる用途向け。1枚だけ渡せば静止画として扱われます。
  - `act` / `learn` / `policy` のいずれも観測を受け付けます。`learn` に経験をバッチ投入した場合、パス指定ぶんは1回のエンコード呼び出しに束ねられます。
  - UI の「🎮 強化学習」タブに常駐状態が出て、**`⏏ VRAM を解放` ボタン**で明示的に落とせます（llama-server にVRAMを譲りたいとき用）。次の推論で自動的に再起動します。
  - エンコーダが起動できない場合、`/policy` は従来経路（`rl_runner.py` が自前でエンコード）に自動フォールバックします。遅くなりますが推論自体は止まりません。

| エンドポイント | 用途 |
|---|---|
| `GET /ml/rl/vjepa2/status` | 常駐状態・ロード済みモデル・呼び出し回数 |
| `POST /ml/rl/vjepa2/embed` | 観測→埋め込み（`paths` か `frames`。`fromAgent` でそのエージェントの学習時設定を使う） |
| `POST /ml/rl/vjepa2/unload` | VRAM を解放（`stopWorker: true` でワーカーごと停止） |

- **🎬 経験ログ用データテーブル（デモ収集）**: 表データの取り込みは「手元にCSVがある」前提ですが、強化学習は**これから溜める**ことの方が多いので、収集用の系統を別に用意しています。「🎮 強化学習」タブは**モデル学習タブと同じ構成**で、`🎬 経験ログ` / `🔍 SQLクエリ` / `🤖 エージェント` のサブタブに分かれています。
  - `🎬 経験ログ` は左に一覧・右に詳細の2ペインで、スキーマとプレビューはモデル学習タブと同じ表示です。加えて**エピソード一覧**（ステップ数・終端フラグ・削除ボタン）が出ます。
  - `🔍 SQLクエリ` は経験ログの中身を読み取り専用で確認できます（モデル学習タブと同じ `/ml/query`）。詳細画面の `🔍 SQLで開く` からそのテーブルの `SELECT` が入った状態で遷移します。行動の偏り（`GROUP BY action`）や観測の欠け（`WHERE frame IS NULL`）を学習前に確認する用途を想定しています。
  - 「離散 / 連続」「観測あり / なし」を選ぶと、必要な列（`episode`, `step`, 状態列, `frame`, 行動列, `reward`, `done`, `recorded_at`）を持つ表ができます。作成直後に**そのスキーマに合わせた記録用のサンプルコード**が表示されます。
  - **観測画像は記録と同じリクエストでアップロードできます**。`steps[].frames` に base64 を入れると `uploads/rl_datasets/<表>/<エピソード>/` に保存され、そのパスが行に埋まります。複数枚渡せばフレーム連番ディレクトリになり、1枚なら静止画として保存されます。ロボット側でファイルを管理する必要はありません。
  - `step` はエピソードごとに自動採番されるので、クライアントは持たなくて構いません。`finishEpisode: true` で最後の行の `done` が立ちます。
  - 失敗したデモは**エピソード単位で削除**できます（行と観測ファイルの両方）。
  - 溜まったらそのまま「➕ エージェントを新規作成」で学習できます。列名がそのまま状態・行動・観測カラムの候補に出ます。

- **データテーブルは用途で分かれています**: 同じ DuckDB に置いたまま、一覧を **📊 モデル学習用** と **🎮 強化学習用** に分離しています。「📊 データ」タブにはモデル学習用だけ、強化学習の学習ダイアログには強化学習用だけが出ます（チェック1つで両方表示にもできます）。
  - CSV で取り込んだ表を後から強化学習用に回せます（データはそのままで、一覧の所属だけが変わる）。「📊 データ」タブの `🎮 強化学習用に移す` / 経験ログダイアログの `📊 戻す` から。
  - 判定は `ml/ml_meta.json` の `kind`。**未設定の既存テーブルは自動的にモデル学習用**として扱われるので、これまでのテーブルの見え方は変わりません。
  - 外部API (`GET /ml/datasets`) は既定で全件返します（互換維持）。`?kind=ml` / `?kind=rl` で絞れます。

- **🌍 世界モデル（V-JEPA 2-AC: 行動条件付き predictor + CEM 計画）**: 「🎮 強化学習」タブの `🌍 世界モデル` サブタブ。経験ログの (観測, 行動, 次の観測) から**行動で潜在がどう変わるか**を学習し、**ゴール画像を1枚渡すだけ**で CEM (交差エントロピー法) が行動系列を探索します。BC と違いタスク変更に再学習が不要で、**成功デモでない雑な操作ログでも学習できます**（方策ではなく力学を学ぶため）。
  - predictor は凍結エンコーダの潜在空間上の**残差MLP**（Δz を学習・出力層ゼロ初期化）。学習時は `rolloutK` ステップの自己回帰で先まで転がして誤差を取り、計画時の多段予測の破綻を抑えます。
  - **正直な評価指標**を自動で出します: `改善比`（恒等ベースライン÷モデル誤差。**1.0x を超えなければ何も学習していない**）と `行動感度`（行動シャッフル時の誤差÷本来の誤差。**1.0x 付近なら行動を無視しており計画に使えない**）。UI のモデルカードに警告として表示されます。
  - 計画は常駐エンコーダ（`vjepa2_server.py` の `/plan`）で実行。エンコードは GPU、predictor のロールアウトは CPU（残差MLPなので十分速く、VRAM を追加で使わない）。MPC として使う場合は返ってきた系列の**1手目だけ実行し、新しい観測で再計画**します。

| エンドポイント | 用途 |
|---|---|
| `POST /ml/rl/ac/train` | 世界モデルの学習（経験ログテーブルから） |
| `GET /ml/rl/ac/models` | 世界モデル一覧（改善比・行動感度つき） |
| `POST /ml/rl/ac/models/:name/plan` | 計画（`framePath`/`frames` + `goalPath`/`goalFrames` → 行動系列） |
| `DELETE /ml/rl/ac/models/:name` | 世界モデル削除 |

```python
# 計画の最小例: ゴール画像に近づく行動系列を探索
r = requests.post(f"{BASE}/ml/rl/ac/models/world_model/plan", headers=H, json={
    "framePath": "rl_datasets/robot_demos/ep001/0000.png",   # 今の観測
    "goalPath": "goals/target.png",                          # こうなってほしい状態
    "horizon": 8,
})
print(r.json()["firstAction"])   # MPC: この1手だけ実行して再計画
```

| エンドポイント | 用途 |
|---|---|
| `GET /ml/datasets?kind=ml\|rl\|all` | テーブル一覧（用途で絞り込み。既定は全件） |
| `PUT /ml/datasets/:name/kind` | テーブルの用途を変更（`{"kind":"rl"}`） |
| `GET /ml/rl/datasets` | 経験ログ用テーブル一覧（行数・エピソード数・最終記録） |
| `POST /ml/rl/datasets/create` | RL スキーマの空テーブルを作成 |
| `POST /ml/rl/datasets/record` | 経験を記録（`table`, `episode`, `steps[]`。観測アップロード込み） |
| `DELETE /ml/rl/datasets/:table/episodes/:episode` | エピソードを削除（行＋観測ファイル） |

```python
# 記録の最小例 (観測画像も一緒に送る)
requests.post(f"{BASE}/ml/rl/datasets/record", headers=H, json={
    "table": "robot_demos", "episode": "ep001", "finishEpisode": True,
    "steps": [{"frames": [b64_png], "action": "grasp", "state": {"joint1": 0.3}}],
})
```
...
| `ml.vjepa2.modelId` | 観測エンコーダの HuggingFace モデルID、デフォルト `facebook/vjepa2-vitl-fpc64-256` (ViT-L/1024次元)。他に `vjepa2-vith-fpc64-256` (0.7B)、`vjepa2-vitg-fpc64-256` / `-384` (1B) |
| `ml.vjepa2.frames` | 1クリップあたりのフレーム数、デフォルト 16。多いほど精度は上がるがエンコードが重くなる |
| `ml.vjepa2.stride` | フレームのサンプル間隔、デフォルト 4 (末尾から `(frames-1)*stride+1` フレーム分を見る) |
| `ml.vjepa2.pooling` | `mean` (全トークン平均、デフォルト) / `mean_last` (最終時刻のトークンのみ。制御タスク向け) |
