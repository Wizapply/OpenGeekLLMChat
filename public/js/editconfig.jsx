...
const CONFIG_HINTS = {
  ...
  'cloudLlm': '外部LLM (有料API) フォールバック。ローカルが限界の時に外部の大きなLLMへ聞き直す',
  'cloudLlm.enabled': 'true で 🌩️ トグル・「外部LLMに聞き直す」ボタンが出る（既定 false。ON にしない限り外部送信は起きない）',
  'cloudLlm.provider': 'openai / anthropic / gemini / openrouter / groq / deepseek / mistral / xai / openai-compatible',
  'cloudLlm.model': 'モデル名（例: gpt-4o, claude-sonnet-5, gemini-2.5-pro, openai/gpt-4o）',
  'cloudLlm.apiKey': 'API キー（外部には公開されない）。空なら apiKeyEnv の環境変数から読む',
  'cloudLlm.apiKeyEnv': 'API キーを読む環境変数名（空ならプロバイダ既定: OPENAI_API_KEY 等）',
  'cloudLlm.baseUrl': 'API のベースURL（空ならプロバイダ既定。openai-compatible では必須）',
  'cloudLlm.label': 'UI に出す表示名（空ならプロバイダ名）',
  'cloudLlm.maxTokens': '1応答の最大出力トークン',
  'cloudLlm.temperature': 'null なら送らない（推論モデルは temperature を拒否することがある）',
  'cloudLlm.timeoutMs': '外部API のタイムアウト（ms）',
  'cloudLlm.maxInputChars': '送信する会話の最大文字数。超えたら古いツール結果から切り詰める',
  'cloudLlm.maxRequestsPerDay': '1日の呼び出し上限（0 で無制限）。うっかり課金の安全網',
  'cloudLlm.sendImages': '添付画像も送る（false で画像を落とす）',
  'cloudLlm.sendSystemPrompt': 'システムプロンプト（役割・ツール案内）も送る',
  'cloudLlm.systemPromptPrefix': '外部LLM にだけ付ける前置き',
  'cloudLlm.maxTokensParam': '空ならプロバイダ既定（openai: max_completion_tokens / 他: max_tokens）',
  'cloudLlm.extraBody': 'リクエスト本文に足す任意キー（例: {"reasoning_effort": "low"}）',
  'cloudLlm.extraHeaders': '追加ヘッダ（OpenRouter の HTTP-Referer 等。外部には公開されない）',
  'cloudLlm.autoEscalate': 'true: ローカルが限界の時、確認の上で自動的に外部LLMへ聞き直す',
  'cloudLlm.autoTriggers': '何を「限界」とみなすか（ctxExhausted / lengthCapped / thinkingOnly / emptyResponse / loopDetected / giveUp / error）',
  'cloudLlm.giveUpPatterns': '「分かりません」判定の正規表現（空なら内蔵の既定）',
  'cloudLlm.giveUpMaxChars': 'この文字数以下の短い回答だけ giveUp 判定の対象にする',
  'cloudLlm.confirmBeforeSend': '送信前にブラウザで確認ダイアログを出す（セッション中1回）',
  'cloudLlm.ragPolicy': 'RAG（登録資料・添付）の扱い: redact=資料を送らない（既定） / abstract=ローカルで一般化した質問だけ送る / send=資料も送る',
  'cloudLlm.privateTools': '結果を外部に送らないツール名（既定: search_documents, search_persistent_documents。read_file 等も追加可）',
  'cloudLlm.privateMarkers': '資料由来メッセージの先頭マーカー（空なら内蔵の既定）',
  'cloudLlm.abstractPrompt': 'abstract: 機密を伏せた一般化質問をローカルLLMに作らせるプロンプト（{question} {context}。空なら既定）',
  'cloudLlm.composePrompt': 'abstract: 外部の回答と資料をローカルで突き合わせるプロンプト（{question} {context} {generalQuestion} {generalAnswer}。空なら既定）',
  'cloudLlm.abstractCompose': 'abstract: 突き合わせをローカルで行う（false なら外部の一般的な回答をそのまま表示）',
  'ml': '機械学習（ML）機能',
  ...
  'tuning.ragDataset': '永続RAGの登録資料からLLMに教師データ(Q&A)を作らせる設定。プロンプトもここで編集できる',
  'tuning.ragDataset.enabled': 'RAGから教師データを作る機能のON/OFF',
  'tuning.ragDataset.poolModel': 'chatModels の名前を入れるとLLMプール管理になり、生成中だけロード→終われば自動アンロード。空ならメインチャットの llama-server をそのまま使う',
  'tuning.ragDataset.endpoint': '生成に使うOpenAI互換エンドポイント（空ならメインチャットの llama-server）。poolModel 設定時は無視',
  'tuning.ragDataset.model': 'リクエストの model フィールド（endpoint 使用時のみ。llama.cpp では無視される）',
  'tuning.ragDataset.mode': '取り込み形式の既定値。closed=資料を渡さず質問だけ（知識注入）/ open=検索結果の体裁で資料を添える（RAG運用の型）/ both=両方',
  'tuning.ragDataset.passageMode': 'パッセージの作り方。auto=ページ情報があればページ単位/page=ページ単位（1ページにつきn問が作れる）/chunks=チャンク単位',
  'tuning.ragDataset.pagesPerPassage': 'ページ単位のとき、1パッセージに何ページぶんを入れるか（1なら1ページ=1パッセージ）',
  'tuning.ragDataset.topUp': '指定数に足りないとき、既出の質問を見せて不足分だけ作り直す（小さいモデルは3問頼んでも1問しか返さないことが多い）',
  'tuning.ragDataset.topUpRounds': '追い生成の最大回数（0で無効）',
  'tuning.ragDataset.passageChunks': '検索用チャンクを何個つないで1パッセージにするか。Q&Aを作るには500文字では文脈が足りないので3前後',
  'tuning.ragDataset.maxPassageChars': '1パッセージの最大文字数（超えたぶんは切る）',
  'tuning.ragDataset.minPassageChars': 'これ未満のパッセージは捨てる（目次・空ページ対策）',
  'tuning.ragDataset.maxPassages': '1ジョブで処理するパッセージ数の上限（0で無制限）。試運転に使う',
  'tuning.ragDataset.questionsPerPassage': '1パッセージあたり作らせるQ&A数',
  'tuning.ragDataset.refusalPerPassage': '「資料に無いこと」を聞く拒否サンプルを1回あたり何件作るか（0で無効）',
  'tuning.ragDataset.refusalEveryNth': '拒否サンプルを作るパッセージ間隔（3なら3パッセージに1回）',
  'tuning.ragDataset.temperature': '生成のランダム性。低すぎると同じ質問ばかりになる（0.3〜0.6推奨）',
  'tuning.ragDataset.verifyTemperature': '検証プロンプトの温度（判定なので0推奨）',
  'tuning.ragDataset.maxTokens': '1パッセージあたりの生成上限',
  'tuning.ragDataset.timeoutSec': 'LLM1回あたりのタイムアウト（秒）',
  'tuning.ragDataset.maxConcurrentJobs': '同時実行ジョブ数（単一GPUなら1）',
  'tuning.ragDataset.verify': '生成したQ&Aを別プロンプトでLLMに検証させる（資料に書かれているか・質問が自立しているか）',
  'tuning.ragDataset.minQuestionChars': 'これより短い質問は捨てる',
  'tuning.ragDataset.minAnswerChars': 'これより短い回答は捨てる',
  'tuning.ragDataset.maxAnswerChars': 'これより長い回答は捨てる（丸ごと転記した回答の除外）',
  'tuning.ragDataset.minCoverage': '回答の文字3-gramが資料にどれだけ含まれるか（0〜1）。下回ると一般論とみなして捨てる',
  'tuning.ragDataset.checkNumbers': '回答の数値が資料に存在するか照合する（捏造・単位換算の検出）',
  'tuning.ragDataset.dedupeThreshold': '質問の重複判定しきい値（0〜1）。高いほど似た質問を残す',
  'tuning.ragDataset.citationKey': 'open形式で使う出典キー（チャットの検索結果と同じ【S1】形式）',
  'tuning.ragDataset.appendCitation': 'open形式の回答末尾に出典キー【S1】を付ける',
  'tuning.ragDataset.jobsFile': '生成ジョブ状態の永続化先',
  'tuning.ragDataset.outputDir': '生成結果(JSONL)の保存先',
  'tuning.ragDataset.styles': '作らせる質問の種類（プロンプトの {styles} に展開される）',
  'tuning.ragDataset.refusalAnswers': '拒否サンプルの回答文（ランダムに選ばれる）。学習後の「知らない時の言い方」になる',
  'tuning.ragDataset.closedSystemPrompt': 'closed形式の学習サンプルに付ける system',
  'tuning.ragDataset.openSystemPrompt': 'open形式の学習サンプルに付ける system',
  'tuning.ragDataset.contextTemplate': 'open形式の入力テンプレート。{key} {source} {context} {question} が展開される。チャットの検索結果と同じ体裁に合わせること',
  'tuning.ragDataset.generatePrompt': 'Q&A生成プロンプト。{passage} {source} {n} {styles} {minChars} {maxChars} が展開される',
  'tuning.ragDataset.refusalPrompt': '拒否サンプル用の質問生成プロンプト。{passage} {source} {n} が展開される',
  'tuning.ragDataset.verifyPrompt': '検証プロンプト。{passage} {question} {answer} が展開される。JSON {"ok":true,"reason":""} で返させる',
};
